...
def truncate_all_by_g(q_t0, q_t1, g, t, y, eps, mu_0, mu_1, truncate_level=0.05):
    """
    ...
    mu_0 = truncate_by_g(np.copy(mu_0.reshape(-1,1)), orig_g, truncate_level)
    mu_1 = truncate_by_g(np.copy(mu_1).reshape(-1,1), orig_g, truncate_level)
    return q_t0, q_t1, g, t, y, eps, mu_0, mu_1

def mse(x, y):
    return np.mean(np.square(x-y))

def regression_loss(q_t0, q_t1, t, y):
    full_q = (1.0 - t) * q_t0 + t * q_t1  # predictions from unperturbed model
    initial_loss = np.sum(np.square(full_q - y))
    return  initial_loss
    ...
    for knob in list(knob_list):
        ...
        for model in ['baseline', 'targeted_regularization']:
            ...
            for rep in range(len(simulation_files)):
                # print(rep)
                file_dir = '../../result/{}/{}/{}/{}'.format(ihdp_dir, knob, rep, model)
                if os.path.exists(file_dir):
                    ...
                    psi_tmle, psi_tmle_std, eps_hat, initial_loss, final_loss, g_loss = psi_tmle_cont_outcome(
                        q_t0, q_t1, g, t,y_dragon, eps, truncate_level=truncate_level)
                    err =  abs(truth - psi_n).mean()
                    tmle_err = abs(truth - psi_tmle).mean()
                    pehe_err = np.sqrt(np.mean(np.square((mu_1 - mu_0) -(q_t1 - q_t0))))
...
def main(ihdp_dir='idhp'):
    ...
    print("--------------------------")
    print(">>>> Results for Non-TMLE estimator:")
    print(dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_base_dir', type=str, help="path to directory LBIDD" , default='IHDP')
